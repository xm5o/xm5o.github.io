...
function mobile(){return matchMedia('(max-width:880px)').matches}
function closeMenu(){document.body.classList.remove('admin-menu-open');$('adminSidebar')?.setAttribute('aria-hidden',mobile()?'true':'false');$('mobileMenuButton')?.setAttribute('aria-expanded','false')}
...
export function navigateWorkspace(name,{scroll=true}={}){
  ...
  document.querySelectorAll('[data-workspace-target]').forEach(button=>{const active=button.dataset.workspaceTarget===name;button.classList.toggle('active',active);if(button.matches('.sidebar-link'))button.setAttribute('aria-current',active?'page':'false');else button.setAttribute('aria-pressed',active?'true':'false')});
  const more=$('mobileMoreButton');if(more){const secondary=['content','seo','library','backups','automation','system'].includes(name);more.classList.toggle('active',secondary);more.setAttribute('aria-pressed',secondary?'true':'false')}
  ...
  storeWorkspace(name);closeMenu();if(scroll)window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});
  window.dispatchEvent(new CustomEvent('immortal:workspace-change',{detail:{workspace:name}}));
}
function openMenu(){document.body.classList.add('admin-menu-open');$('adminSidebar')?.setAttribute('aria-hidden','false');$('mobileMenuButton')?.setAttribute('aria-expanded','true');requestAnimationFrame(()=>document.querySelector('.sidebar-link.active')?.focus())}
function syncViewport(){if(!mobile()){document.body.classList.remove('admin-menu-open');$('adminSidebar')?.setAttribute('aria-hidden','false');$('mobileMenuButton')?.setAttribute('aria-expanded','false')}else if(!document.body.classList.contains('admin-menu-open'))$('adminSidebar')?.setAttribute('aria-hidden','true')}
export function initAdminLayout(){
  ...
  window.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.body.classList.contains('admin-menu-open')){$('mobileMenuButton')?.focus();closeMenu()}});
  ...
  syncViewport();const saved=storedWorkspace();navigateWorkspace(workspaceMeta[saved]?saved:'overview',{scroll:false});
}
