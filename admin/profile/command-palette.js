...
const commands=[
  ...
  {label:'Open local staging preview',icon:'bx-window-open',run:()=>window.open('stage.html','_blank','noopener')},
  {label:'Show what’s new',icon:'bx-git-branch',run:()=>$('#releaseButton')?.click()},
  {label:'Admin appearance preferences',icon:'bx-slider-alt',run:()=>goAnd('system',()=>$('#adminPreferencesCard')?.scrollIntoView({behavior:'smooth',block:'center'}))},
