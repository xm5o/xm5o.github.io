...
function injectSlots() {
  ...
  box.innerHTML = `<div class="slots-copy"><small>Commission slots</small><strong>${available === 0 ? 'Full right now' : `Open · ${available}/${total} available`}</strong></div><div class="slots-dots" aria-label="${used} of ${total} commission slots used">${Array.from({ length: total }, (_, index) => `<span class="${index < used ? 'used' : 'open'}"></span>`).join('')}</div>`;
...
function renderPortfolioDetails(title) {
  ...
  panel.innerHTML = `<div class="portfolio-detail"><small>Service</small><strong>${details.service}</strong></div><div class="portfolio-detail"><small>Engine</small><strong>${details.engine}</strong></div><div class="portfolio-detail"><small>Type / level</small><strong>${details.level}</strong></div><div class="portfolio-detail portfolio-focus"><small>What I worked on</small><p>${details.focus}</p></div>`;
...
function getSubmissionEndpoint() {
  let override = '';
  try { override = String(localStorage.getItem('immortal-commission-api-url') || '').trim(); } catch {}
...
function collectRequest(builder) {
  ...
  return { service: get('service', 'Charting'), project: get('project'), engine: get('engine'), length: get('length'), difficulty: get('difficulty'), deadline: get('deadline', 'Flexible / not provided'), details: get('details', 'No extra details yet'), estimate: String(builder.dataset.chartEstimate || '').trim(), currency: String(builder.dataset.chartCurrency || '').trim(), honeypot: get('_contact'), page: location.href.split('#')[0] };
}
...
function validateSubmission(payload, builder, status) {
  const required = [['project', 'Add the song or project name first.'], ['engine', 'Add the engine and version first.'], ['length', 'Add the song length first.'], ['difficulty', 'Add the difficulty or style first.']];
...
async function submitCommission(endpoint, payload) {
  ...
  try {
    const response = await fetch(`${endpoint}/api/commission`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), signal: controller.signal });
...
function setupDirectSubmission() {
  ...
  if (!endpoint || !/^https:\/\//i.test(endpoint)) { button.dataset.submitMode = 'discord-fallback'; return; }
  ...
  button.addEventListener('click', async event => {
    ...
    try {
      ...
      if (status) status.textContent = result.id ? `Request sent successfully. Reference: ${result.id}` : 'Request sent successfully.';
      try { localStorage.removeItem('immortal-commission-draft-v2'); } catch {}
      setTimeout(() => { button.disabled = false; button.innerHTML = original; }, 3500);
    } catch (error) {
...
function init() {
  ...
  import('../scripts/site-settings.js?v=20260915-1').catch(error => console.warn('[Commission] Managed settings unavailable:', error?.message || error));
}
