...
for(const[viewportName,viewport]of viewports){const page=await browser.newPage({viewport});await page.addInitScript(()=>localStorage.setItem('immortal-site-language-v1','en'));const errors=[];page.on('pageerror',e=>errors.push(e.message));for(const[name,url]of pages){await page.goto(`${base}${url}`,{waitUntil:'domcontentloaded',timeout:30000});await page.addStyleTag({content:'*,*::before,*::after{animation:none!important;transition:none!important;caret-color:transparent!important}html{scroll-behavior:auto!important}'}).catch(()=>{});await page.waitForTimeout(900);await page.evaluate(()=>window.scrollTo(0,0));const file=`${name}-${viewportName}.png`,target=path.join(out,file);await page.screenshot({path:target,fullPage:true,animations:'disabled'});await compare(file,target);const overflow=await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+4);if(!overflow)failures.push(`${name}-${viewportName}: horizontal overflow detected`);if(viewportName==='mobile'){const readability=await page.evaluate((pageName)=>{const selectors={commission:['.hero-lead','.section-heading p','.service-card p'],selina:['.selina-sub','.selina-shot-card p','.command-desc'],'selina-case-study':['.case-lede','.case-panel p','.case-feature-copy p','.case-step p'],privacy:['body'],analytics:['.analytics-hero p','.analytics-card .label','.analytics-privacy-note'],'not-found':['.error-subtitle']}[pageName]||[];return selectors.map(selector=>{const el=document.querySelector(selector);return el?{selector,size:parseFloat(getComputedStyle(el).fontSize)}:null}).filter(Boolean)} ,name);for(const item of readability){if(item.size<14)failures.push(`${name}-mobile: ${item.selector} is only ${item.size.toFixed(1)}px`)}const touchTargets=await page.evaluate((pageName)=>{const selectors={commission:['.hero-actions .button'],selina:['.selina-cta-group .enhanced-btn'],'selina-case-study':['.case-button'],privacy:['button'],'not-found':['.back-button']}[pageName]||[];return selectors.map(selector=>{const el=document.querySelector(selector);if(!el)return null;const r=el.getBoundingClientRect();return{selector,width:r.width,height:r.height}}).filter(Boolean)},name);for(const item of touchTargets){if(item.height<44)failures.push(`${name}-mobile: ${item.selector} touch target is only ${item.height.toFixed(1)}px tall`)}}}if(errors.length)failures.push(`${viewportName}: page errors: ${errors.join(' | ')}`);await page.close()}
