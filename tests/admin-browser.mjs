...
await page.keyboard.press('/');await page.waitForSelector('#adminGlobalSearch:not([hidden])');await page.locator('#adminSearchInput').fill('Profile');await page.waitForSelector('.admin-search-result');assert((await page.locator('.admin-search-result').count())>=1,'Global admin search returned no local results.');await page.keyboard.press('Escape');
await page.keyboard.press('g');await page.keyboard.press('c');await page.waitForFunction(()=>document.getElementById('workspaceTitle')?.textContent==='Content');
await page.locator('[data-workspace-target="system"]').first().click();await page.waitForSelector('[data-workspace="system"]:not([hidden])');await page.waitForSelector('#adminReleaseChannel');await page.locator('#adminReleaseChannel').selectOption('beta');assert(await page.evaluate(()=>document.documentElement.dataset.adminChannel==='beta'),'Beta release channel did not apply.');await page.locator('#adminReleaseChannel').selectOption('stable');
...
await page.goto(`${base}/admin/profile/stage.html`,{waitUntil:'domcontentloaded'});await page.waitForSelector('#stageFrame');await page.waitForFunction(()=>/staged change/.test(document.getElementById('stageStatus')?.textContent||''),null,{timeout:15000});await page.screenshot({path:'artifacts/admin-staging.png',fullPage:true});
await page.goto(`${base}/admin/profile/`,{waitUntil:'domcontentloaded'});await page.waitForSelector('#draftCount');await page.evaluate(async()=>{const d=await import('/admin/profile/draft.js');d.clearDraft()});await page.waitForTimeout(250);
await page.setViewportSize({width:390,height:844});await page.reload({waitUntil:'domcontentloaded'});await page.waitForSelector('.mobile-bottom-nav');assert(await page.locator('.mobile-bottom-nav').evaluate(el=>getComputedStyle(el).display!=='none'),'Mobile bottom navigation is hidden.');assert(await noHorizontalOverflow(),'Mobile admin has horizontal overflow.');await page.locator('.mobile-bottom-nav [data-workspace-target="profile"]').click();assert(await page.locator('#workspaceTitle').textContent()==='Profile','Mobile profile navigation failed.');await page.locator('#mobileMoreButton').click();assert(await page.evaluate(()=>document.body.classList.contains('admin-menu-open')),'Mobile More button did not open sidebar.');await page.screenshot({path:'artifacts/admin-mobile.png',fullPage:true});
